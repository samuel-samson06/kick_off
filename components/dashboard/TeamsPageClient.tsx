...
import { addSubscription, removeSubscription } from "@/lib/services/subscriptions";
...
export default function TeamsPageClient({
  ...
  const [subscribedTeamIds, setSubscribedTeamIds] = useState(initialSubscribedTeamIds);
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                {filteredTeams.map((team) => (
                  <NationalTeamCard
                    ...
                    disabled={
                      (!subscribedTeamIds.includes(team.id) && subscribedTeamIds.length >= 5) ||
                      savingTeamIds.includes(team.id)
                    }
                  />
                ))}
              </div>
  ...
  return (
    <article className="rounded-[28px] border border-lime-400/20 bg-[radial-gradient(circle_at_top,rgba(163,230,53,0.14),transparent_42%),linear-gradient(180deg,rgba(24,24,27,0.98),rgba(18,18,18,0.98))] p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-xs font-bold uppercase tracking-[0.24em] text-zinc-400">Team</div>
