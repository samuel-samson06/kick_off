...
type MatchData = {
  ...
  homeTeam: string;
  awayTeam: string;
  kickoffIn: { days: string; hours: string; minutes: string };
  date: string;
  time: string;
};

export default function MainMatch({ matchData }: { matchData: MatchData | null }) {
  if (!matchData) {
    return (
      ...
      <div className="mt-10 flex flex-col sm:flex-row items-center md:items-end justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex items-end gap-1 sm:gap-2 md:gap-3 font-black text-white">
            <TimeBlock value={matchData.kickoffIn.days} label="Days" />
            <Separator />
            <TimeBlock value={matchData.kickoffIn.hours} label="Hrs" />
            <Separator />
            <TimeBlock value={matchData.kickoffIn.minutes} label="Mins" />
          </div>
        </div>

        <div className="sm:text-right">
          <div className="text-2xl md:text-3xl font-bold text-white sm:text-[1.9rem]">{matchData.date}</div>
          <div className="mt-1 text-sm max-sm:text-center text-zinc-300">{matchData.time}</div>
        </div>
      </div>
    </div>
  );
